...
public class LiveJbossEapWebServicesIssueTest {
    ...
    private static final Logger logger = LoggerFactory.getLogger(LiveJbossEapWebServicesIssueTest.class);
    ...
    public static void startSimpleProcess( CommandWebService commandWebService, URL deploymentUrl, String user ) throws Exception {
        ...
        // webService
        JaxbProcessInstanceResponse jpir = doWebserviceRequest(commandWebService, spc, "start process",
    ...
    private static void doBadAuthTest( URL deploymentUrl, Command cmd ) {
        ...
        assertTrue("Unauthorized webservice actions should not have succeeded! [" + unauthClientCreated + "/"
                + unauthClientCallSucceeded + "]", !unauthClientCreated && !unauthClientCallSucceeded);
    }

    private static <T> T doWebserviceRequest( CommandWebService service, Command<?> cmd, String oper, Class<T> respClass )
            throws Exception {
    ...
    private static CommandWebService createDefaultClient( URL deploymentUrl, String user, String pwd ) throws Exception {
        CommandWebService client = RemoteRuntimeEngineFactory.newCommandWebServiceClientBuilder().addServerUrl(
                deploymentUrl).addUserName(user).addPassword(pwd).addDeploymentId(KJAR_DEPLOYMENT_ID).addExtraJaxbClasses(
                        MyType.class).buildBasicAuthClient();
