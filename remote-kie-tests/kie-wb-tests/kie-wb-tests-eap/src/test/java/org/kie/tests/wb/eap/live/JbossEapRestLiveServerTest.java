...
public class JbossEapRestLiveServerTest extends KieWbWarJbossEapDeploy {

    URL deploymentUrl;

    public JbossEapRestLiveServerTest() { 
        try {
            deploymentUrl = new URL("http://localhost:8080/kie-wb/");
        } catch (MalformedURLException e) {
            fail("Unable to create url: " + e.getMessage());
        }
    }
...
//        createAndDeployTestKJarToMaven();
    ...
    public void testRemoteApiGetTaskInstance() throws Exception { 
        ClientRequestFactory requestFactory = createBasicAuthRequestFactory(deploymentUrl, USER, PASSWORD);
        restTests.remoteApiGetTaskInstance(requestFactory, deploymentUrl, USER, PASSWORD);
    }
}